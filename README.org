...
* News
  ...
  - Note taken on [2019-12-08 Sun 11:21] \\
...
Global_Config& set_log(const std::function<void(const char* const msg)>&);
Global_Config& set_log(); // removes logger
bool log() const; // checks if there is an active logger
